...
Chrome (optional, gfs ≥ v0.2.1): **`GFS_BRAND_SUB`** (app-bar tag; `-` hides), **`GFS_FOOTER`** (authenticated footer; `-` hides), **`GFS_LOGIN_SIMPLE=true`** (plain `/login`). See [Traefik README](../docker-compose/traefik/README.md#chrome-optional).
